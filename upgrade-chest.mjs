#!/usr/bin/env node
/**
 * upgrade-chest.mjs — CONK v8 upgrade PTB
 *
 * Adds chest.move (Primitive 8 of 8) to axiom_tide on Sui mainnet.
 * Same pattern as upgrade-siren.mjs — bypasses the Sui CLI bug where
 ...
 *   node upgrade-chest.mjs --dry-run   # simulate first
 *   node upgrade-chest.mjs             # live deploy
 *
 * After success: send Franklin the new package ID printed at the end.
 */
...
const CURRENT_PACKAGE = '0xb4220c9faa4e6be4b557d3c81772f96ef90d5688009887a1c73ef9b7eaa6917c'; // v7
...
function loadKeypair() {
  ...
  if (!raw) {
    console.error([
      ...
      '    4. node upgrade-chest.mjs --dry-run',
      '',
    ].join('\n'));
    process.exit(1);
  }
...
async function main() {
  ...
  console.log('✅  Cap matches — v7 confirmed, upgrading to v8\n');
  ...
  if (published) {
    ...
    console.log('\n    → DM Franklin the new package ID so he can update SDK + demo.');
  } else {
